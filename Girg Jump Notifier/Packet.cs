...
namespace Girg_Jump_Notifier
{
    ...
    public class Packet
    {
        ...
        public void SetPtr(int position)
        {
            _ptr = position;
        }

        public void SetPtr(byte[] search)
        {
            // Find where the pattern starts
            var position = findSequence(Build(), search);
            // Set the current position to the pattern start
            SetPtr(position);
        }

        private static int findSequence(byte[] charBin, byte[] search)
        {
            // current sequence to compare
            var current = new byte[search.Length];
            // search until the last comparable sequence
            var maxSearchRange = charBin.Length - search.Length - 1;
